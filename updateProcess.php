...
if(isset($_SESSION["p"])){
    ...
    Database::iud("UPDATE `product` SET `title`='".$title."',`qty`='".$qty."',`delivery_fee_weston`='".$dwc."',`delivery_fee_other`='".$doc."',`description`='".$description."' WHERE `id`='".$pid."'");
        ...
        for($x = 0;$x < $length;$x++){
            if(isset($_FILES["i".$x])){
                ...
                if(in_array($file_type,$allowed_img_extentions)){
                    ...
                    Database::iud("INSERT INTO `product_img`(`path`,`product_id`) VALUES ('".$file_name."','".$pid."')");
